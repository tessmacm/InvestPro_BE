...
builder.Services.AddDbContext<ApplicationDbContext>((serviceProvider, options) =>
{
    ...
    if (httpContext != null)
    {
        ...
        var customEnv = httpContext.Request.Headers["X-Environment"].ToString();

        bool isStaging = (!string.IsNullOrEmpty(origin) && origin.Contains("staging.tessmaims.co.uk", StringComparison.OrdinalIgnoreCase)) ||
                         (!string.IsNullOrEmpty(referer) && referer.Contains("staging.tessmaims.co.uk", StringComparison.OrdinalIgnoreCase)) ||
                         (!string.IsNullOrEmpty(host) && host.Contains("staging.tessmaims.co.uk", StringComparison.OrdinalIgnoreCase)) ||
                         string.Equals(customEnv, "staging", StringComparison.OrdinalIgnoreCase);
...
using (var scope = app.Services.CreateScope())
{
    ...
    foreach (var conn in connectionStringsToInit)
    {
        ...
        {
            logger.LogError(ex, "An error occurred seeding or migrating DB: {Conn}", conn);
        }
    }
}
